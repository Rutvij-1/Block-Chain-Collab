...
contract AveragePriceAuction {
    ...
    modifier alreadyBidder(uint256 auction_id) {
        require(Auctions[auction_id].bidded[msg.sender] == true , "Didn't place a bet,no point in revealing the bid");
        _;
    }
    ...
    modifier auctionEnded(uint256 auction_id){
        require(Auctions[auction_id].ended == true ,"Cant Ask refund,auction not ended");
        _;
    }
        ...
        alreadyBidder(auction_id)
    {
        ...
        bool success = false;
            ...
            emit DepositNotEnough(auction_id,msg.sender);
            }
    ...
    function placeBid(uint256 auction_id,address bidder, uint value) internal
        ...
        auctionActive(auction_id)
    {
        if(Auctions[auction_id].no_of_bids == 0){
            ...
            for (uint256 i = 0 ; i < Auctions[auction_id].no_of_bids ; i++){
                ...
                uint256 bid_value = Auctions[auction_id].pendingReturns[bidder_address];
                uint256 difference = Auctions[auction_id].sum - bid_value*Auctions[auction_id].no_of_bids;
                if(difference < 0){
